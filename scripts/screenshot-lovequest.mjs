 ...
 *
 * Usage:
 *   npm run screenshot:lovequest              # local dev (http://127.0.0.1:5173)
 *   npm run screenshot:lovequest:prod         # production Vercel URL
 *
 *   node scripts/screenshot-lovequest.mjs https://example.com
 *
 * PowerShell (env var):
 *   $env:SCREENSHOT_BASE_URL="https://couple-rpg-app.vercel.app"; npm run screenshot:lovequest
 */
...
const cliUrl = process.argv[2]?.startsWith('http') ? process.argv[2] : null;
const BASE_URL = (cliUrl || process.env.SCREENSHOT_BASE_URL || 'http://127.0.0.1:5173').replace(
  /\/$/,
  ''
);
